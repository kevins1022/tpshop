 ...
 /**
  ...
  if(IS_POST){
   ...
   $order=M("order");
    $cancel=D("cancel");
   $cancel->create();
   $cancel->update_time=NOW_TIME;
   $cancel->info="系统取消";
    $cancel->status=3;
	//保存数据
   $cancel ->where("id='$id'")->save();
 
	$act="<A href='http://".$_SERVER['HTTP_HOST']."/index.php?s=/Home/Order/canceldetail/id/".$id."'>取消详情</a>";
   ...
   if(IS_POST){
		...
		$cancel->info="系统拒绝";
           $cancel->update_time = NOW_TIME;
			/* 编辑后保存编辑人*/
			  $cancel->status=2;
           $result=$cancel->where("id='$id'")->save();
 $order=M("order");
 $info = M('cancel')->find($id);
$orderid=$info["orderid"];
  //更新订单列表订单状态为已取消，清空取消订单操作
 $act="<A href='http://".$_SERVER['HTTP_HOST']."/index.php?s=/Home/Order/canceldetail/id/".$orderid."'>被拒绝</a>";
   $data = array('status'=>'5','act_cancel'=>$act);//设置订单为订单已取消
   $order->where("orderid='$orderid'")->setField($data);
                if($result){
                    //记录行为
                    action_log('update_cancel', 'cancel', $data['id'], UID);
                    $this->success('更新成功', Cookie('__forward__'));
