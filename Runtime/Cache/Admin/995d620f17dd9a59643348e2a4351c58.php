...
<body>
	...
	<div class="main-title">
		<h2>退货管理 [ <?php if(isset($_GET['group'])): ?><a href="<?php echo U('index');?>">全部</a><?php else: ?><strong>全部</strong><?php endif; ?>&nbsp;<?php if(is_array($group)): foreach($group as $key=>$vo): if(($group_id) != $key): ?><a href="<?php echo U('index?group='.$key);?>"><?php echo ($vo); ?></a><?php else: ?><strong><?php echo ($vo); ?></strong><?php endif; ?>&nbsp;<?php endforeach; endif; ?> ]</h2>
	</div>
	...
	<div class="cf">
        ...
        <div class="search-form fr cf">
            <div class="sleft">
                <input type="text" name="orderid" class="search-input" value="<?php echo I('orderid');?>" placeholder="请输入订单号">
                <a class="sch-btn" href="javascript:;" id="search" url="/index.php?s=/Admin/Back/index.html"><i class="btn-search"></i></a>
            </div>
        </div>
	</div>
			...
			<tbody>
				<?php if(!empty($list)): if(is_array($list)): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$order): $mod = ($i % 2 );++$i;?><tr>
					<td><input class="ids row-selected" type="checkbox" name="id[]" value="<?php echo ($order["id"]); ?>"></td>
						...
						<td><a title="编辑" href="<?php echo U('edit?id='.$order['id'].'&pid='.$pid);?>"><?php echo (get_good_name($order["goodid"])); ?></a></td>
...
<a title="同意" href="<?php echo U('agree?id='.$order['id']);?>">同意</a>
                         <a title="拒绝" href="<?php echo U('refuse?id='.$order['id']);?>">拒绝</a>  
